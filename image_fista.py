...
parser.add_argument("--save_dir", type=str, default='./puca/image_fista', help='save directory')
...
for i in tqdm.tqdm(range(opt.test_num)):
    image_result1.append(Fista_torch(get_sparse_echo[i],get_phase_imag[i,...],get_phase_mese[i,...]))
    image_result2.append(Fista_torch(get_complete_echo[i],get_phase_imag[i,...],get_phase_mese[i,...]))
t2 = time.time()
...
savemat(os.path.join(opt.save_dir,opt.save_name),{'sparse_echo':get_sparse_echo.cpu().numpy(),'complete_echo':get_complete_echo.cpu().numpy(),
                                                       'sparse_imag':image_result1.cpu().numpy(),'complete_imag':image_result2.cpu().numpy(),
                                                       'phase_imag':get_phase_imag.cpu().numpy(),'phase_mese':get_phase_mese.cpu().numpy()})
